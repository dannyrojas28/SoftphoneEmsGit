...
- (void)cancel:(LinphoneChatMessage*)chatMessage {
    if (chatMessage) {
        [[LinphoneManager.instance fileTransferDelegates] removeObject:self];
        LinphoneChatMessage *msg = chatMessage;
        if (_message != NULL) {
            if (linphone_chat_message_get_storage_id(_message) == linphone_chat_message_get_storage_id(chatMessage)) {
                msg = _message;
                _message = NULL;
            }
        }
